...
Then poll until every CI check has a final status (not `pending` or `in_progress`). Poll every 30 seconds, timeout after 10 minutes:
...
Fix each finding, add a new commit, push, and return to step 8 to wait for checks again. Repeat until Sonar reports 0 new issues.
...
- Repeat until CodeRabbit raises no new unresolved threads on the latest push.
...
### 12. Rebase onto master (if needed)
