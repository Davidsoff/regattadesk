...
- Persist tile upload lifecycle state in metadata (`pending`, `failed`, `ready`) with idempotent retry semantics for unreliable networks, and derive the operator workspace lifecycle (`pending`, `syncing`, `partial_failure`, `failed`, `completed`) from that metadata so retryability is visible without recapture.
